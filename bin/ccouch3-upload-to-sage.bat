...
if defined TOGOS_FBS_SSH_HOSTNAME (set "command=%command% -command-server:togos-fbs plink -batch %portarg% tog@%TOGOS_FBS_SSH_HOSTNAME% ccouch3 command-server -repo:$(cat /mnt/sage/tog/.ccouch/repo-name.txt) /mnt/sage/tog/.ccouch -sector %ccouch_store_sector% ";"" && set any_remotes=1)
